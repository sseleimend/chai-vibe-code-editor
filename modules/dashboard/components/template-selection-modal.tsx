  ...
  const handleCreateProject = () => {
    if (selectedTemplate) {
      const templateMap: Record<
        string,
        "REACT" | "ANGULAR" | "VUE" | "NEXT" | "EXPRESS" | "HONO"
      > = {
        react: "REACT",
        angular: "ANGULAR",
        vue: "VUE",
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredTemplates.length > 0 ? (
                    filteredTemplates.map((template) => (
                      <div
                        key={template.id}
                        className={`relative border rounded-lg flex p-6 cursor-pointer hover:scale-[1.02] transition-all duration-300 ${
                          selectedTemplate === template.id
                            ? "border-[#E93F3F] shadow-[0_0_0_1px_#E93F3F,0_8px_20px_rgba(233,63,63,0.15)]"
                            : "hover:border-[#E93F3F] shadow-[0_2px_10px_rgba(0,0,0,0.08)] hover:shadow-[0_10px_30px_rgba(233,63,63,0.15)]"
                        }`}
                        role="button"
                        tabIndex={0}
