...
export const useFileExplorer = create<FileExplorerState>((set, get) => ({
  ...
  openFile: (file) => {
    ...
    if (existingFile) {
      set({ selectedFileId: fileId, editorContent: existingFile.content });
      return;
    }
  ...
  closeFile: (fileId) => {
    ...
    let newActiveFileId = selectedFileId;
    ...
    set({
      openFiles: newFiles,
      selectedFileId: newActiveFileId,
  ...
  handleRenameFile: async (
    ...
    const { templateData, openFiles, selectedFileId } = get();
    if (!templateData) return;
    ...
    try {
      ...
      if (fileIndex !== -1) {
        ...
        set({
          ...
          selectedFileId:
            selectedFileId === oldFileId ? newFileId : selectedFileId,
        });
  ...
  updateFileContent: (fileId, content) => {
    set((state) => ({
      ...
      editorContent:
        fileId === state.selectedFileId ? content : state.editorContent,
    }));
  },
}));
